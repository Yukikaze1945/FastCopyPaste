...
本项目由三个部分组成：.NET 8 WinForms 常驻 Host、原生 x64 `IExplorerCommand` Shell 扩展，以及当前用户安装脚本。Host 只在资源管理器文件列表中接管带文件剪贴板的 `Ctrl+V`；Shell 扩展在 Windows 11 第一层菜单提供“FastCopy 粘贴到这里”。实际复制或移动始终交给用户配置的现有 `FastCopy.exe`。
