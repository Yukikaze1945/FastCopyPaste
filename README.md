...
FastCopy Paste 是一个面向 64 位 Windows 10/11 的资源管理器集成工具。它可在资源管理器文件列表中通过自定义快捷键把文件剪贴板操作交给 FastCopy。
...
本项目不支持 Windows 10 Build 19041 以下版本、32 位系统、ZIP/MTP/回收站等虚拟目录。FastCopy 自带的右键菜单保持独立，本程序不会修改它。

## 安装

1. 从 GitHub Releases 下载 `FastCopyPaste-current-user.zip`，先解压完整 ZIP。
...
程序安装到 `%LOCALAPPDATA%\Programs\FastCopyPaste`，配置与日志保存到 `%LOCALAPPDATA%\FastCopyPaste`。安装脚本会注册当前用户登录启动项并启动托盘 Host；从 1.2.0 或更早版本升级时还会注销已经停用的旧菜单包。重复运行可安全覆盖安装。
...
卸载会移除启动项、安装目录、本工具配置/日志以及旧版本遗留的菜单包注册，不会删除或修改 FastCopy。

## 从源码构建

构建环境需要 .NET 8 或更新的 SDK：

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\Build.ps1
```

脚本会先运行单元测试，再发布自包含 .NET Host，最终生成 `artifacts\FastCopyPaste-current-user.zip`。发布包不包含 PDB、FastCopy 或用户配置。
...
- 常驻 Host 只会在按下所配置的快捷键后解析当前 Explorer 文件视图，不会向 Explorer 注入代码。
...
- 未签名的程序可能触发 SmartScreen 或 PowerShell 来源提示。
- Windows 更新可能改变 Explorer 焦点结构；遇到问题请附上 Host 日志提交 Issue。
